@php
    $route = 'products';
    $filters = ['element' => $products_elements, 'search' => true, 'date' => true, 'datesince' => true];
    $order = ['fecha de creación' => 't_products.created_at', 'título' => 't_products.name'];
@endphp
        ...
        <div class="table" id="table-container">
            ...
            @foreach($products as $product_element)
                <div class="table-row swipe-element">
                    <div class="table-field-container swipe-front" data-swipe="{{$product_element->id}}">
                            <div class="table-field"><p><span>Name:</span> {{$product_element->name}}</p></div>
                            <div class="table-field"><p><span>Element:</span> {{$product_element->element->name}}</p></div>
                            <div class="table-field"><p><span>Date:</span> {{$product_element->created_at}}</p></div>
                    </div>

                    <div class="table-icons-container swipe-back">
                        <div class="table-icons edit-button right-swipe" data-url="{{route('products_show', ['product' => $product_element->id])}}" data-swipe="{{$product_element->id}}">
                            <svg viewBox="0 0 24 24">
                                <path d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z" />
                            </svg>
                        </div> 
                        
                        <div class="table-icons delete-button left-swipe" data-url="{{route('products_destroy', ['product' => $product_element->id])}}" data-swipe="{{$product_element->id}}">
                            <svg viewBox="0 0 24 24">
...
@isset($product)
    ...
    <div class="form-container">
        <form class="admin-form" id="form-faqs" action="{{route("products_store")}}" autocomplete="off">
            ...
            <div class="tabs-container">
                ...
                <div class="tabs-container-buttons">
                    ...
                    <div class="sub-menu-buttons">
                        <div id="clear-button" data-url="{{route('products_create')}}">
                ...
                <div class="two-columns">
                    <div class="form-group">
                        ...
                        <div class="form-input">
                            <select name="category_id" data-placeholder="Seleccione una elemento" class="input-highlight" id="category-select">
                                <option></option>
                                @foreach($products_elements as $product_element)
                                    <option value="{{$product_element}}" {{$product_element == $product_element->id ? 'selected':''}} class="category_id">{{ $product_element->name }}</option>
                                @endforeach
                            </select>                   
                        </div>
                    </div>
